...
Live autonomous trading bot on Oracle Cloud VM. 16 modules, 236 tests. See `skills/kalshi/README.md` for full module details, test commands, and deployment instructions.

---

## Architecture Overview

OpenClaw-based agent with 6 skills (notion, email, calendar, canvas, github, kalshi). Skills are ES module directories under `/skills/`, each with `index.js` and `SKILL.md`. OpenClaw handles sessions, memory, and Telegram messaging.

## Tech Stack

Node.js 22+, pnpm, ES modules (`"type": "module"`). Tests use `node:test` + `node:assert/strict` (zero test dependencies). Dependencies: @notionhq/client, googleapis, nodemailer, dotenv.

## Key Patterns

- All skills return `{ success: true/false, error?: string, ...data }`
- Pure functions exported from `client.js` in each skill (validateEnv, parseDate, logAction, etc.)
- API credentials loaded via `import 'dotenv/config'` from `.env`
- Notion is the single source of truth for tasks — never infer schema dynamically
...
# All unit tests
node --test skills/*/tests/*.test.js

# Kalshi unit tests only (no credentials needed)
node --test skills/kalshi/tests/config.test.js skills/kalshi/tests/budget.test.js
```

## What NOT to Read

- `node_modules/`, `archive/` — waste tokens
- `*.csv`, `*.pem`, credential JSON files — secrets
- `skills/kalshi/README.md` — only read when working on kalshi specifically
- `docs/` in bulk — reference specific files
